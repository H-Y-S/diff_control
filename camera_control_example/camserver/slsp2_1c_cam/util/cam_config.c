...
#ifdef SLS42_CAMERA
DBG(1, "SLS42_CAMERA software configuration\n");
#endif
#ifdef SLS8x2_CAMERA
DBG(1, "SLS8x2_CAMERA software configuration\n");
#endif
#ifdef SLSDET_CAMERA
DBG(1, "SLSDET_CAMERA software configuration\n");
#endif
...
for (i = 0; i < cam_config_count; i++)
	...
	/* start of main loop switch - please preserve the format for mkheader */
		switch (cam_config_list[i].cmnd)
			...
			case Ioc_host:				// specific to SLS
				isolate_string(&p);
				strcpy(iocHost, p);
				break;

			case Base_trim_all_pattern:
				sscanf(p, "%i", &base_trim_all_pattern);
				break;

			case Base_read_chip_pattern0:
				sscanf(p, "%i", &base_read_chip_pattern0);
				break;

			case Base_read_chip_pattern1:
				sscanf(p, "%i", &base_read_chip_pattern1);
				break;

			case Base_enable_pattern:
				sscanf(p, "%i", &base_enable_pattern);
				break;

			case Base_disable_pattern:
				sscanf(p, "%i", &base_disable_pattern);
				break;

			case I2c_pattern_base:
				sscanf(p, "%i", &i2c_pattern_base);
				break;

			case Base_vpg_ena_pattern:
				sscanf(p, "%i", &base_vpg_ena_pattern);
				break;				

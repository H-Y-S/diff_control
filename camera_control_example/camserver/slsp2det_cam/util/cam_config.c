...
{
char line2[LENFN], cmsg[LENFN]="", dmsg[LENFN]="", *p;
FILE *ifp;
struct stat buf;

#if defined SLSP2DET_CAMERA
DBG(1, "SLSP2DET_CAMERA software configuration\n");
#elif
 #error
#endif
...
if ((ifp = fopen(SYSTEM_CAM_CONFIG_FILE, "r")) != NULL)	// e.g. /usr/local/etc/camrc
	{
	sprintf(cmsg, "...(1)Configured from %s\n", SYSTEM_CAM_CONFIG_FILE);
	DBG(1, "...(1)Configuring from %s\n", SYSTEM_CAM_CONFIG_FILE);
	resource_path = SYSTEM_CAM_CONFIG_FILE;
	read_configuration(ifp, dmsg);
	fclose(ifp);
	}
else
	DBG(1, "***Did not find system config file: %s\n", SYSTEM_CAM_CONFIG_FILE);
...
if ((ifp = fopen(line2, "r")) != NULL)
	{
	sprintf(cmsg, "...(2)Configured from %s\n", line2);
	DBG(1, "...(2)Configuring from %s\n", line2);
	resource_path = line2;
	read_configuration(ifp, dmsg);
	fclose(ifp);
	}
else
	DBG(1, "***Did not find user config file: %s\n", USER_CAM_CONFIG_FILE);
...
if ((ifp = fopen(line2, "r")) != NULL)
	{
	sprintf(cmsg, "...(3)Configured from %s\n", line2);
	DBG(1, "...(3)Configuring from %s\n", line2);
	resource_path = line2;
	read_configuration(ifp, dmsg);
	fclose(ifp);
	}
else
	DBG(1,"***Did not find local config file: %s\n", LOCAL_CAM_CONFIG_FILE);
...
{
int i;
char rmsg[LENFN], line[LENFN];
//double v;

for (i = 0; i < cam_config_count; i++)
	{
	
	if (!strncasecmp(p, cam_config_list[i].name, strlen(cam_config_list[i].name)))
		{
		DBG(6, "Configuring: %s\n", p);
...
\*****************************************************************************/
			...
			case Cam_threshold_setting:
				set_calibration_state(p);
				break;

			case Cam_gapfill:
				gap_fill_byte = atoi(p);
				initialize_theImage(gap_fill_byte);
				break;

			case Cam_badpixmap:
				load_bad_pixel_list(p);
				break;

			case Cam_flatfield:
				load_flat_field_correction_file(p);
				break;

			case Cam_ackinterval:
				acknowledge_interval=atoi(p);
				break;

			case Cam_trim_directory:
				strcpy(trim_directory, p);
				break;
#if 0
			case Cam_ExpPeriod:
				sscanf(p, "%lf", &v);
			...
			case Cam_NExpFrame:
				i = atoi(p);
				dcb_set_exposure_count_limit(i);
				break;
...
\*****************************************************************************/
			...
			case Cam_definition_file:
				...
				camera_read_setup(line, rmsg);		// in interface.c
				if (*rmsg)
					{
...
line2[0]='\0';
read_configuration(ifp, line2);
fclose(ifp);

if(controllingProcess)
	xor_tab_initialize(-1);		// incorporate possibly changed tau
